...
int SCRFD_TRT::draw(cv::Mat& rgb, cv::Mat &faceFilter, const std::vector<FaceObject>& faceobjects, int alpha_slider)
{
    cv::Mat smallFilter, filterClone;
    cv::Mat rgb_obj, rgb_rect;
    
    faceFilter.copyTo(filterClone);

    std::cout << "faceobjects : " << faceobjects.size()  << " " << rgb.cols << " " << rgb.rows << std::endl;
    ...
    //size_t i = 1;
    {
        ...
        cv::resize(filterClone, smallFilter, cv::Size(obj.rect.width, obj.rect.height));
        //rgbRoi = rgb(obj.rect);

        cv::Rect faceobject_roi(obj.rect.x, obj.rect.y, obj.rect.width, obj.rect.height);
        cv::Mat rgbRoi(rgb, faceobject_roi);
        cv::Mat rgbObj = rgb(faceobject_roi).clone();
        
        //std::cout << "rgbRoi "  << " " << rgbRoi.cols << " " << rgbRoi.rows << std::endl;
        smallFilter.copyTo(rgbRoi);

        std::cout << "pos2 :  " << alpha_slider  << std::endl;

        alpha = (double) alpha_slider/alpha_slider_max ;
        beta = ( 1.0 - alpha );
        addWeighted( rgbObj, alpha, rgbRoi, beta, 0.0, rgbRoi);
